using  Statistics, Distributions, MCMCChains, LinearAlgebra, Phylo, FASTX, MolecularEvolution, CodonMolecularEvolution, Plots, EllipticalSliceSampling, AbstractMCMC, NNlib, StatsBase, ProgressMeter
"""
...
function to_probability_vector(model::SKBDIModel, ambient_sample::Vector{Float64})
    ...
    for i in 1:model.suppression_dim
        probability_vector[model.masks[i, :]] .*= model.transition_functions[i](suppression_parameters[i])
    end
...
function calculate_alloc_grid_and_theta(model::GeneralizedFUBARModel, ambient_samples::Vector{Vector{Float64}}, burnin::Int=0)
    """
